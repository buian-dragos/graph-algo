...
class Service:
    ...
    def first_not_visited(self,visited,paths,ct, v):
        out_edge = self.repo.dOut
        o_e = tuple(out_edge.keys())
        for vert in o_e:
            if vert == v:
                for e in out_edge[v]:
                    if e not in visited:
                        ...
                        if len(paths) > 0:
                            ...
                            return out_edge[v], self.repo.edges[f"{v},{e}"]
                        else:
                            return out_edge[v], self.repo.edges[f"{v},{e}"]
        return False,False,False


    def asg3(self,s,x,k):                                                                    # NOQA
        if self.out_degree(s) == 0 or self.in_degree(x) == 0:
        ...
        for i in range(0,100000):
            ...
            current_path = []
            visited = []
            while ct < k and (current_cost < min_cost or min_cost == -1):
                ...
                if current == x:
                    ...
                    break
                not_vis = self.first_not_visited(visited,paths,ct,current)
                if self.out_degree(current) == 0 or len(not_vis) == 3:
                    print(visited)
                    if len(visited) > 0:
                        current = visited[-1]
                    ...
                    current = not_vis[0]
                    current_cost += int(not_vis[1])
                visited.append(current)
            paths.append(current_path)

        print(f"{min_cost}       {min_path}")
